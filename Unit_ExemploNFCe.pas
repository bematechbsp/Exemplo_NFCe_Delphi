unit Unit_ExemploNFCe;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, ExtCtrls, jpeg, Buttons, ComCtrls, ShellApi, Unit_VendeItemCompleto,
  Unit_DadosConsumidorNFCe, Unit_FormaPagamentoNFCe, Unit_FormaPagamento, Unit_AbreCupom,
  Unit_TabRetornoEstendido, Unit_CancelaCupom, ActnList, Unit_MensagemTerminaCupom,
  Unit_RetornoInfNFCe, Unit_EfetuaFormaPgtoNFCeEx, Unit_Vinculado, Unit_AdicInfoComb,
  UnitAtencao, Unit_IniciaFechamentoCupom;

type
  TForm_Principal = class(TForm)
    GroupBox1: TGroupBox;
    AbreCupomMFD: TButton;
    ...
    TabRetornoEstendido: TButton;
    VendeItemCompleto: TButton;
    DadosConsumidorNFCe: TButton;
    EfetuaFormaPagamentoNFCe: TButton;
    EfetuaFormaPagamento: TButton;
    ProtocoloUltimaNFCe: TButton;
    GroupBox2: TGroupBox;
    Label5: TLabel;
    ...
    ComboBox1: TComboBox;
    Label2: TLabel;
    CancelaCupomFechado: TButton;
    RetornaInformacoesNFCe: TButton;
    NumeroSerieNFCe: TButton;
    NumeroNotaNFCe: TButton;
    Label6: TLabel;
    BtEfetuaFormaPgtoNFCeEx: TPanel;
    ...
    Button9: TButton;
    EditEmail: TEdit;
    ComboBoxDadosEmissao: TComboBox;
    Button16: TButton;
    EfetuaFormaPgtoNFCeEx: TButton;
    AdicionaInformacoesCombustivel: TButton;
    AcionaGaveta: TButton;
    procedure AbreCupomMFDClick(Sender: TObject);
    procedure IniciaFechamentoCupomClick(Sender: TObject);
    procedure EfetuaFormaPagamentoNFCeClick(Sender: TObject);
    procedure TerminaFechamentoCupomClick(Sender: TObject);
    procedure SairClick(Sender: TObject);
    procedure LimparClick(Sender: TObject);
    procedure CancelaCupomAbertoClick(Sender: TObject);
    ...
    procedure ImprimeCupomCompleto;
    procedure TabRetornoEstendidoClick(Sender: TObject);
//    procedure FormCreate(Sender: TObject);
    ...
    procedure ProtocoloUltimaNFCeClick(Sender: TObject);
    procedure VendeItemCompRegSimplesClick(Sender: TObject);
    procedure CancelaCupomFechadoClick(Sender: TObject);
    procedure RetornaInformacoesNFCeClick(Sender: TObject);
    procedure NumeroSerieNFCeClick(Sender: TObject);
    procedure NumeroNotaNFCeClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure CupomVinculadoClick(Sender: TObject);
    procedure SpeedButton2Click(Sender: TObject);
    procedure ComboBoxEmissaoChange(Sender: TObject);
    procedure CupomCompletoClick(Sender: TObject);
    procedure EfetuaFormaPgtoNFCeExClick(Sender: TObject);
    procedure AdicionaInformacoesCombustivelClick(Sender: TObject);
    procedure AcionaGavetaClick(Sender: TObject);
  private
...
var
  ...
  Retorno, RetFuncao, TipoLayout, Layout, cCPF : string;
  iConta : integer;
...
function Bematech_FI_AbreCupomMFD( CGC_CPF: String; cNome: String; cEndereco: String ): Integer; StdCall; External 'BEMAFI32.DLL';
...
function Bematech_FI_EfetuaFormaPagamentoIndiceDescricaoForma( FormaPagamento: string; ValorFormaPagamento: string; DescricaoFormaPagto: string ): integer; StdCall; External 'BEMAFI32.DLL';
function Bematech_FI_FlagsFiscaisStr( FlagFiscal: String ): Integer; StdCall; External 'BEMAFI32.DLL';
function Bematech_FI_CancelaCupomMFD(CPF: String; Name: String;Adress: String): Integer; StdCall; External 'BEMAFI32.DLL';
...
// JÚLIO CÉSAR BRUNO
//Skype: Bematech.plataformafiscal
//==========================================

//++++++++PROCEDURE CUPOMCOMPLETO++++++++++++++++++++++++++++++++++++
procedure TForm_Principal.ImprimeCupomCompleto;
Var
cNome, cEndereco, MensagPromo,
paymentForm, value, licensingCNPJ, licensingCode,
authorizationCode, integrationCode, forma, tipo, valor : string;

begin
  cCPF := '80871740540';
  cNome := '';
  cEndereco := '';
  iRetorno := Bematech_FI_AbreCupomMFD( pchar( cCPF ), pchar( cNome ), pchar( cEndereco ) );

  VendeItemCompleto.Click;

forma := 'Dinheiro';
tipo := '%';
valor := '0';
iRetorno := Bematech_FI_IniciaFechamentoCupom( pchar( forma ), pchar( tipo ), pchar( valor ) );

  paymentForm       := 'Cartao Credito';
value             := '';
licensingCNPJ     := '';
licensingCode     := '01';
authorizationCode := '';
integrationCode   := '1';
iRetorno := Bematech_FI_EfetuaFormaPagamentoNFCeEx	(	paymentForm, value, licensingCNPJ, licensingCode, authorizationCode, integrationCode);

MensagPromo := 'Obrigado, volte sempre!';
iRetorno := Bematech_FI_TerminaFechamentoCupom( MensagPromo );
Memo2.Clear;
...
Memo1.Lines.Add('Mensagem: '+MensagPromo);
Memo1.Lines.Add('----------------------------------------------------------');
end;
...
procedure TForm_Principal.AbreCupomMFDClick(Sender: TObject);

begin
Form_AbreCupom := TForm_AbreCupom.Create(Application);
Form_AbreCupom.ShowModal;

if flagAbre = 1 then
begin
 iRetorno := Bematech_FI_AbreCupomMFD( sCPF, sNome, sEndereco );
 Memo1.Clear;
 Memo1.Lines.Add('Abre Cupom');
 Memo1.Lines.Add('CPF: '+sCPF);
 Memo1.Lines.Add('Nome: '+sNome);
 Memo1.Lines.Add('Endereço: '+sEndereco);
 Memo1.Lines.Add('----------------------------------------------------------');
end
else
begin
 Memo1.Lines.Add('Função AbreCupomMFD cancelada...');
end;
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
if ST3 = 66 then
  ShowMessage('Redução Z pendente!');
if ST3 = 7 then
  ShowMessage('Cupom Fiscal Aberto!');
Memo2.Clear;
 Memo2.Lines.Add('AbreCupomMFD: '+ IntToStr(iRetorno));
Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
Memo2.Lines.Add('ST3: '+ IntToStr(ST3));
...
TipoEmissao := IntToStr(ComboBoxDadosEmissao.ItemIndex);
...
FormIniciaFechamentoCupom := TFormIniciaFechamentoCupom.Create(Application);
FormIniciaFechamentoCupom.ShowModal;
...
procedure TForm_Principal.EfetuaFormaPagamentoNFCeClick(Sender: TObject);
begin
Form_PagamentoNFCe := TForm_PagamentoNFCe.Create(Application);
Form_PagamentoNFCe.ShowModal;
end;
...
FormTerminaFechamentoCupom := TFormTerminaFechamentoCupom.Create(Application);
FormTerminaFechamentoCupom.ShowModal;

if flagTerm = 1 then
begin
  iRetorno := Bematech_FI_TerminaFechamentoCupomNFCe( MensagPromo, Tax );
  Memo1.Lines.Add('Mensagem: '+MensagPromo);
  Memo1.Lines.Add('Total de tributos: '+Tax);
  Memo1.Lines.Add('----------------------------------------------------------');
  Memo2.Clear;
  Memo2.Lines.Add('TerminaFechamentoCupom: '+IntToStr(iRetorno));
  iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
...
begin
  Memo1.Lines.Add('Função TerminaFechamentoCupomNFCe cancelada...');
end;
end;
...
procedure TForm_Principal.CancelaCupomAbertoClick(Sender: TObject);
begin
...
Memo1.Lines.Add('Alerta! Ocorrência não contida nos retornos da SEFAZ, código: '+Status);
end;
Memo1.Lines.Add('----------------------------------------------------------');
Memo2.Clear;
...
cIndice := IntToStr(ComboBox1.ItemIndex); {contador será programado:
                                    ...
                                    2 - CRO
                                    3 - CCF}
cContador := Edit6.Text; //valor do contador a ser programado.
...
begin
FormAtencao := TFormAtencao.Create(Application);
...
if eXecZ = 2 then
  ...
  begin
   ...
   ReducaoZ.Click;
   Memo1.Clear;
   Memo1.Lines.Add('Aguarde redução Z...');
   Sleep(8000);
...
procedure TForm_Principal.TabRetornoEstendidoClick(Sender: TObject);
begin
FormRetEstendido := TFormRetEstendido.Create(Application);
FormRetEstendido.ShowModal;
end;
...
//var
//cFormaPgto, cDescricao, cValorPago : string;
...
procedure TForm_Principal.ProtocoloUltimaNFCeClick(Sender: TObject);
var
cProtocolo : string;
cDataHora  : string;
Protocol, DateHour : string;
iConta     : integer;
begin

for iConta := 1 to 16 do cProtocolo := cProtocolo + ' ';
for iConta := 1 to 20 do cDataHora := cDataHora + ' ';
Protocol := cProtocolo;
DateHour := cDataHora;
iRetorno := Bematech_FI_ProtocoloUltimaNFCe ( cProtocolo, cDataHora);
Memo1.Lines.Add('Último protocolo: '+Protocol);
Memo1.Lines.Add('Data e hora: '+DateHour);
Memo1.Lines.Add('----------------------------------------------------------');
Memo2.Clear;
Memo2.Lines.Add('ProtocoloUltimaNFCe: '+IntToStr(iRetorno));
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
Memo2.Lines.Add('Retorno ProtocoloUltimaNFCe: ');
Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
Memo2.Lines.Add('ST3: '+ IntToStr(ST3));
...
procedure TForm_Principal.VendeItemCompRegSimplesClick(Sender: TObject);
var
Codigo: string;
EAN13: string;
Descricao: string;
...
procedure TForm_Principal.CancelaCupomFechadoClick(Sender: TObject);
var
CPF    : string;
Name   : string;
Adress : string;
...
begin
FormCancelaCupomFechado := TFormCancelaCupomFechado.Create(Application);
FormCancelaCupomFechado.ShowModal;

CPF    := FormCancelaCupomFechado.Edit1.Text;
Name   := FormCancelaCupomFechado.Edit2.Text;
Adress := FormCancelaCupomFechado.Edit3.Text;
iRetorno := Bematech_FI_CancelaCupomMFD(CPF, Name, Adress);
Memo2.Clear;
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
Memo2.Lines.Add('Retorno CancelaCupom: ');
Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
...
procedure TForm_Principal.RetornaInformacoesNFCeClick(Sender: TObject);
var
paramType , paramValue, retChaveAcesso, retSerie, retNumNFCe, retCancelled,
retSendStatus, retSendProtocol, retSendProtocolDatetime, retCancellationStatus,
retCancellationProtocol : string;

begin
FormRetornoInfNFCe := TFormRetornoInfNFCe.Create(Application);
FormRetornoInfNFCe.ShowModal;
{1=COO, 2=CCF, 3=Chave de Acesso, 4=NNF}
paramType := TipoForma;
paramValue := ParametroForma;
//paramType := '1';
//paramValue := '230';
...
Memo2.Clear;
Memo2.Lines.Add('Retorno RetornaInformacoesNFCe: '+ IntToStr(iRetorno));
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
...
Memo1.Clear;
Memo1.Lines.Add('Tipo: '+TipoFormaDesc);
Memo1.Lines.Add('Parâmetro: '+paramValue);
Memo1.Lines.Add('Chave de Acesso: '+retChaveAcesso);
Memo1.Lines.Add('Nº Série: '+retSerie);
Memo1.Lines.Add('Nº NFC-e: '+retNumNFCe);
Memo1.Lines.Add('Cancelado? '+retCancelled);
Memo1.Lines.Add('Código de Status: '+retSendStatus);
Memo1.Lines.Add('Nº Protocolo: '+retSendProtocol);
Memo1.Lines.Add('Data e hora do protocolo: '+retSendProtocolDatetime);
Memo1.Lines.Add('Status Cancelamento: '+retCancellationStatus);
Memo1.Lines.Add('Protocolo de Cancelamento: '+retCancellationProtocol);
end;

procedure TForm_Principal.NumeroSerieNFCeClick(Sender: TObject);
var
serialNumber : string;
iConta : integer;

begin
for iConta := 1 to 3 do serialNumber := serialNumber + ' ';
iRetorno := Bematech_FI_NumeroSerieNFCe(serialNumber);
Memo1.Lines.Add('Nº de Série: '+serialNumber);

Memo2.Clear;
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
Memo2.Lines.Add('Retorno NumeroSerieNFCe: '+IntToStr(iRetorno));
Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
Memo2.Lines.Add('ST3: '+ IntToStr(ST3));
...
procedure TForm_Principal.NumeroNotaNFCeClick(Sender: TObject);
var
noteNumber : string;
iConta : integer;

begin
for iConta := 1 to 6 do noteNumber := noteNumber + ' ';
iRetorno := Bematech_FI_NumeroNotaNFCe(noteNumber);
Memo1.Lines.Add('Nº da NFC-e: '+noteNumber);

Memo2.Clear;
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
Memo2.Lines.Add('Retorno NumeroNotaNFCe: '+ IntToStr(iRetorno));
Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
Memo2.Lines.Add('ST3: '+ IntToStr(ST3));
...
begin
//Habilitando o retorno estendido ao iniciar a aplicação.
iRetorno := Bematech_FI_HabilitaDesabilitaRetornoEstendidoMFD('1');
end;

procedure TForm_Principal.CupomVinculadoClick(Sender: TObject);
var
cTexto, NumeroCupom : string;
iConta : integer;

begin
FormVinculado := TFormVinculado.Create(Application);
FormVinculado.ShowModal;
{for iConta := 1 to 7 do NumeroCupom := NumeroCupom + ' ';
iRetorno := Bematech_FI_NumeroCupom( NumeroCupom );}
NumeroCupom := '410';

iRetorno := Bematech_FI_AbreComprovanteNaoFiscalVinculado( pchar( formaPag ), pchar(valorPag), pchar (NumeroCupom) );
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
if (ACK = 6) and (ST3 = 0) then
  begin 
  cTexto := 'SI                                   Rede 5'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'MU                    Codigo transacao: 200'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'LA                    Codigo operacao: 2000'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := '........S...I...M...U...L...A...D...O......'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'SI                         NSU SiTef: 60011'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'MU                           06/11/15 14:18'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'LA                         ID PDV: SE000001'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'DO                  Estab.: 000000000000005'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := '.......S...I...M...U...L...A...D...O.......'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'SI                          Host: 999060011'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := 'MU              Transacao Simulada Aprovada'+#13+#10;
   iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  cTexto := '                   (SiTef)                 '+#13+#10;
  iRetorno := Bematech_FI_UsaComprovanteNaoFiscalVinculado( pchar( cTexto ) );
  iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
  if (ACK = 6) and (ST3 = 0) then
    ...
    begin
        ShowMessage('Erro ST3 = '+IntToStr(ST3)+', favor verificar!');
    end
  end
 else
 begin
...
if flag <> 1 then
begin
Memo1.Lines.Add('ìndice do item: '+itemIndex);
Memo1.Lines.Add('Código produto ANP: '+ANPProductCode);
Memo1.Lines.Add('UF de consumo: '+consumeUF);
end
else
begin
Memo1.Lines.Add('ìndice do item: '+itemIndex);
Memo1.Lines.Add('Código produto ANP: '+ANPProductCode);
Memo1.Lines.Add('UF de consumo: '+consumeUF);
Memo1.Lines.Add('% de mistura de GN: '+percentMixGN);
Memo1.Lines.Add('Código CODIF: '+CODIF);
Memo1.Lines.Add('Qtde do produto vendido à temperatura ambiente: '+quantity);
Memo1.Lines.Add('Base de Cálculo CIDE: '+BCProductCIDE);
Memo1.Lines.Add('Alíquota CIDE: '+taxProductCIDE);
Memo1.Lines.Add('Valor CIDE: '+valueCIDE);
Memo1.Lines.Add('Número do Bico: '+fuelNozzleNumber);
Memo1.Lines.Add('Número da bomba: '+fuelPumpNumber);
Memo1.Lines.Add('Número do tanque: '+fuelTankNumber);
Memo1.Lines.Add('Valor Inicial do Encerrante: '+fuelGaugeInitial);
Memo1.Lines.Add('Valor Final do Encerrante: '+fuelGaugeFinal);
end;

Memo2.Clear;
iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
Memo2.Lines.Add('Retorno AdicionaInformacoesCombustivel: '+IntToStr(iRetorno));
Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
Memo2.Lines.Add('ST3: '+ IntToStr(ST3));
...
begin
Memo1.Lines.Add('AdicionaInformacoesCombustivel cancelado...')
end;
end;

procedure TForm_Principal.ComboBoxEmissaoChange(Sender: TObject);
{
0 - Sem emissão
1 - Somente impressão
2 - Somente envio de e-mail
3 - Somente geração de arquivo PDF
4 - Impressão + envio de e-mail
5 - Impressão + geração de arquivo PDF
6 - Impressão + envio de e-mail + geração de arquivo PDF
7 - Envio de e-mail e geração de arquivo PDF
}
begin
if ComboBoxDadosEmissao.ItemIndex = 7 then
  EditEmail.Enabled := True;
  EditEmail.Color := clWhite;
if ComboBoxDadosEmissao.ItemIndex = 4 then
  EditEmail.Enabled := True;
  EditEmail.Color := clWhite;
if ComboBoxDadosEmissao.ItemIndex = 2 then
  EditEmail.Enabled := True;
  EditEmail.Color := clWhite;
...
for z := 1 to x do
ImprimeCupomCompleto;
end;

procedure TForm_Principal.EfetuaFormaPgtoNFCeExClick(Sender: TObject);
begin
//flagEx := 1;
FormEfetuaPagamentoNFCeEx := TFormEfetuaPagamentoNFCeEx.Create(Application);
FormEfetuaPagamentoNFCeEx.ShowModal;
{descBandeira := 'Outros';
valorForma := '679,00';
CNPJCrede := '60419645000195';
bandeira := '99';
CodAuto := '';
CodIntegra := '2';}
if flagEx = 1 then
begin
  iRetorno := Bematech_FI_EfetuaFormaPagamentoNFCeEx(DescForma, valorForma, CNPJCrede, bandeira, CodAuto, CodIntegra);
  Memo1.Lines.Add('Descrição Forma: '+DescForma);
  Memo1.Lines.Add('Valor: '+valorForma);
  Memo1.Lines.Add('CNPJ Cred.: '+CNPJCrede);
  Memo1.Lines.Add('Bandeira: '+bandeira);
  Memo1.Lines.Add('Cód. Autorização: '+CodAuto);
  Memo1.Lines.Add('Cód. Integração: '+CodIntegra);
  Memo2.Clear;
  iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
  Memo2.Lines.Add('Retorno EfetuaFormaPagamentoNFCeEx: '+IntToStr(iRetorno));
  Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
  Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
  Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
  Memo2.Lines.Add('ST3: '+ IntToStr(ST3));
...
begin
  Memo1.Lines.Add('Execução da EfetuaFormaPagamentoNFCeEx cancelada...');
end;
end;

procedure TForm_Principal.AdicionaInformacoesCombustivelClick(
  Sender: TObject);
begin
FormAdicInfoCombustivel := TFormAdicInfoCombustivel.Create(Application);
FormAdicInfoCombustivel.ShowModal;
...
percentMixGN, CODIF, quantity, consumeUF, BCProductCIDE, taxProductCIDE, valueCIDE,
fuelNozzleNumber, fuelPumpNumber, fuelTankNumber, fuelGaugeInitial, fuelGaugeFinal);
 Memo1.Lines.Add('Nº do item: '+itemIndex);
 Memo1.Lines.Add('Código ANP: '+ANPProductCode);
 Memo1.Lines.Add('Percent. Mix. GN:: '+percentMixGN);
 Memo1.Lines.Add('CODIF: '+CODIF);
 Memo1.Lines.Add('Quantidade: '+quantity);
 Memo1.Lines.Add('UF de consumo: '+consumeUF);
 Memo1.Lines.Add('BC CIDE: '+BCProductCIDE);
 Memo1.Lines.Add('Aliquota CIDE: '+taxProductCIDE);
 Memo1.Lines.Add('Valor CIDE: '+valueCIDE);
 Memo1.Lines.Add('Nº do bico: '+fuelNozzleNumber);
 Memo1.Lines.Add('Nº da bomba: '+fuelPumpNumber);
 Memo1.Lines.Add('Nº do tanque: '+fuelTankNumber);
 Memo1.Lines.Add('Nº Valor Inicial Encerrante: '+fuelGaugeInitial);
 Memo1.Lines.Add('Nº Valor Final Encerrante: '+fuelGaugeFinal);
  Memo2.Clear;
  iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
  Memo2.Lines.Add('Retorno AdicionaInformacoesCombustivel: '+IntToStr(iRetorno));
  Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
...
begin
   Memo1.Lines.Add('Função AdicionaInformacoesCombustivel cancelada...');
end;
end;

procedure TForm_Principal.AcionaGavetaClick(Sender: TObject);
begin
 iRetorno := Bematech_FI_AcionaGaveta();
 Memo2.Clear;
  iRetorno := Bematech_FI_RetornoImpressoraMFD( ACK, ST1, ST2, ST3 );
  Memo2.Lines.Add('Retorno AcionaGaveta: '+IntToStr(iRetorno));
  Memo2.Lines.Add('ACK: '+ IntToStr(ACK));
  Memo2.Lines.Add('ST1: '+ IntToStr(ST1));
  Memo2.Lines.Add('ST2: '+ IntToStr(ST2));
  Memo2.Lines.Add('ST3: '+ IntToStr(ST3));
